...
LOAD DATA LOCAL INFILE "beijingAdmin.csv" INTO TABLE abase COLUMNS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '' ESCAPED BY '"' LINES TERMINATED BY '\n' (@col1,@col2,@col3,@col4) set nid=@col1,name=@col2,lat=@col4,lng=@col3;
...
LOAD DATA LOCAL INFILE "apoint-at" INTO TABLE anode COLUMNS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '' ESCAPED BY '"' LINES TERMINATED BY '\n' (@col1,@col2,@col3,@col4,@col5,@col6) set nid=@col1,dev_num=@col2,rec_num=@col3,seg=DATE_ADD("2016-07-05", INTERVAL @col4 HOUR);
